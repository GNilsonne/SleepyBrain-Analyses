...
%     end
    ...
    for condnr=1:3,
       ...
       % but now only 1:st second is modelled
    for condnr=4:7,
        durations{1,condnr}=repmat(0, (size(onsets{1,condnr})));
    end
    ...
    % Write file
    save(fullfile('/data/stress/ARROWS/Onsetfiles_stimuli0s/', strcat(folder(1:3), '_', num2str(session), folder(4:10))),'names','onsets','durations');
end