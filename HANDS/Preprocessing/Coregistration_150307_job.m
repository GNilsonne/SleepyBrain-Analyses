...
for i = 1:length(folders)
    ...
    anatomical = dir(strcat('/data/stress/Structural_HANDSAll/', subject, '*001.nii'));
    structural = arrayfun(@(file) strcat('/data/stress/Structural_HANDSAll/', file.name), anatomical, 'UniformOutput', false);
    mean = dir(strcat('/data/stress/HANDS_AGE/45Slices/', subject_session, '/m*.nii'));
    meanEPI = arrayfun(@(file) strcat('/data/stress/HANDS_AGE/45Slices/', subject_session, '/', file.name), mean, 'UniformOutput', false);
...
for j = 1:length(folders)
    ...
    anatomical = dir(strcat('/data/stress/Structural_HANDSAll/', subject, '*001.nii'));
    structural = arrayfun(@(file) strcat('/data/stress/Structural_HANDSAll/', file.name), anatomical, 'UniformOutput', false);
    mean = dir(strcat('/data/stress/HANDS_AGE/46Slices/', subject_session, '/m*.nii'));
    meanEPI = arrayfun(@(file) strcat('/data/stress/HANDS_AGE/46Slices/', subject_session, '/', file.name), mean, 'UniformOutput', false);
...
P=load('/data/stress/HANDS_AGE/46Slices/427_2/rp_a20140317_180658MRC303GustavNilsonnes007a1001_001.txt’)
