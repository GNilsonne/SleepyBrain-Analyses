% Does not work with NAs. Remove if necessary
...
for i = 1:length(folders)
    ...
    subject = str2num(folders(i).name(1:3));
    ...
    ratings = strcat('/data/stress/HANDS_AGE/Ratings_HANDS/', num2str(subject), '.', num2str(session), '.csv');
    ...
    pmod(1).name{1}  = 'RatedUnpleasantnessPain';
    pmod(1).param{1} = E(:, 2);
    pmod(1).poly{1}  = 1;   
    ...
    % Write file
    save(fullfile('/data/stress/HANDS_AGE/Onsetfiles_modulated', strcat(folder(1:3), '_', num2str(session), folder(4:10))),'names','onsets','durations','pmod');
end