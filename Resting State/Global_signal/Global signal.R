# Script to analyse global signal
...
# ISI
lm4 <- lme(globalsignal_logsd ~ condition * AgeGroup + ISI + FD, data = data_glob, random = ~1|ID)
summary(lm4)
intervals(lm4)

# ESS
lm5 <- lme(globalsignal_logsd ~ condition * AgeGroup + ESS + FD, data = data_glob, random = ~1|ID)
summary(lm5)
intervals(lm5)

# KSQ sleep quality
lm6 <- lme(globalsignal_logsd ~ condition * AgeGroup + KSQ_SleepQualityIndex + FD, data = data_glob, random = ~1|ID)
summary(lm6)
intervals(lm6)

# Sleep measures: analysed separately for the two conditions
# Total sleep time
lm7 <- lme(globalsignal_logsd ~ TST * AgeGroup + FD, data = subset(data_glob, data_glob$condition == "fullsleep"), random = ~1|ID, na.action = na.omit)
summary(lm7)
intervals(lm7)
plot(effect("TST*AgeGroup", lm7))

lm8 <- lme(globalsignal_logsd ~ TST * AgeGroup + FD, data = subset(data_glob, data_glob$condition == "sleepdeprived"), random = ~1|ID, na.action = na.omit)
summary(lm8)
intervals(lm8)
plot(effect("TST*AgeGroup", lm8))


#TST, REM, St1 sleep, St2 sleep, SWS , stage changes, spectral power in the delta band, number of awakenings, sleep efficiency, and WTSP within and between sleep conditions.
