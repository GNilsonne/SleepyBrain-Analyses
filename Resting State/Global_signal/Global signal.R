...
lm23b <- lme(globalsignal_logsd ~ delta_t_n3_p25_m_use_nsd/totpow_n3_p25_m_use_nsd * AgeGroup + FD, data = subset(data_glob, data_glob$condition == "fullsleep"), random = ~1|ID, na.action = na.omit)
summary(lm23b)
intervals(lm23b)
#plot(effect("delta_t_n3_p25_m_use_nsd/totpow_n3_p25_m_use_nsd*AgeGroup", lm23b))

lm24b <- lme(globalsignal_logsd ~ delta_t_n3_p25_m_use_nsd/totpow_n3_p25_m_use_nsd * AgeGroup + FD, data = subset(data_glob, data_glob$condition == "sleepdeprived"), random = ~1|ID, na.action = na.omit)
summary(lm24b)
intervals(lm24b)
#plot(effect("delta_t_n3_p25_m_use_nsd/totpow_n3_p25_m_use_nsd*AgeGroup", lm24b))

lm25c <- lme(globalsignal_logsd ~ (condition + AgeGroup) * delta_t_n3_p25_m_use_nsd/totpow_n3_p25_m_use_nsd + FD, data = subset(data_glob), random = ~1|ID, na.action = na.omit)
summary(lm25c)
intervals(lm25c)
