...
for ses=1:2; %ses in this case actually refers to comdditions,
    ...
    %some  partiicpants excluded due to motion or other issues (check!)
    if ses == 1
        ex = [20 48 49]; % 9028 bad normalisation, 9066-1 (does not exist. Actually this session is named wrongly. Changed manually in GLMatthe moment) 9067-1 (head motion)
        condir = [basedir 'GLMs/glm_t-tests/group_sleepdeprived/'];
    else
        ex = [20 28]; % 9028 (bad normalisation) 9038-2 (signal loss), 
