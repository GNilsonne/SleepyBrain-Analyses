...
for ses=1:2;
    ...
    condir = [basedir 'GLMs/glm_t-tests/PPI_amy_R_group_ses' num2str(ses) '/'];
    mkdir(condir)
    ...
    for cdx = 1:size(con_names,1) % loop over contrast numbers
        ...
        for pdx = 1:size(subs,1)
            if isempty(find(ex == pdx))
                ...
                if cdx < 10
                    job.des.t1.scans(n) = {[basedir  'GLMs/glm_t-tests/sub-' num2str(subs(pdx)) '_' num2str(s(ses)) '/PPI_amy_R/con_000' num2str(cdx) '.nii,1']};
                else
                    job.des.t1.scans(n) = {[basedir  'GLMs/glm_t-tests/sub-' num2str(subs(pdx)) '_' num2str(s(ses)) '/PPI_amy_R/con_00' num2str(cdx) '.nii,1']};
                end
                n=n+1;
            end
        end
