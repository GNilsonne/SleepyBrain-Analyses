...
% to work properly. 
for idx =49:size(subs,1)
    disp(subs(idx,:))
    for sess =1:2
        cd([base '/ds201_R1.0.0/sub-' num2str(subs(idx,:)) '/ses-' num2str(sess)])
        try
            %cd anat/
            %gunzip *.gz
            %cd ../fmap
...
for pdx = 1:size(subs,1)
    disp(subs(pdx,:)) % to keep track of progress
...
% 
% for pdx = 20:size(subs,1)
%  disp(subs(pdx,:)) % to keep track of progress
   
   % coregister vdm files to epi
   %coreg_vdm(base, ['sub-' num2str(subs(pdx,:))],bt1(pdx));
% end



for pdx = 31:size(subs,1)
   % preproc 1 does slicetime and unwarping without! field maps. vdm files
   ...
   %preproc1_qc(base, ['sub-' num2str(subs(pdx,:))])
end
...
&sleep_MPFC_PPI(pt)
%sleep_group_test_PPI

for pdx = 1:size(subs,1)
   disp(subs(pdx,:)) % to keep track of progress
   for sess = 1:2
       pt = ['sub-' num2str(subs(pdx,:)), '_' num2str(sess)]
       %sleep_MPFC_PPI(pt)
       sleep_amy_L_PPI(pt)
       sleep_amy_R_PPI(pt)
   end
...
sleep_group_test_PPI
sleep_group_test_PPI_amy_L
sleep_group_test_PPI_amy_R

% extract ROIs from pre-defined regions
%Require sleep_ons.mat
load('sleep_ons.mat')
